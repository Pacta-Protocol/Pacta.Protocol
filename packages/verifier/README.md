...
pacta-verify receipt.json --pubkey 0x04… --rpc https://sepolia.base.org --sender 0x…
...
- **Merkle tree**: leaves are the raw 32-byte `entry_hash` values in `seq`
  order; parent = `SHA-256(left || right)`; a level with an odd node count
  duplicates its last node; a single leaf is its own root. Proof steps are
  `{ hash, pos }` siblings walked leaf → root.
